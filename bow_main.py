...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    vPoints = 0 #None  # numpy array, [nPointsX*nPointsY, 2]

    # todo

    x = np.array(range(8, img.shape[1]-8))
    y = np.array(range(8, img.shape[0] - 8))
    x = np.linspace(x[0], x[-1], nPointsX, dtype=int)
    y = np.linspace(y[0], y[-1], nPointsY, dtype=int)

    # print(x)
    # print(y)

    Y, X = np.meshgrid(y, x)
    vPoints = np.vstack([Y.ravel(), X.ravel()])

    # print(vPoints.transpose().shape)

    return vPoints.transpose() # reshape to right dimension output

# img = cv2.imread("/Users/dominikcydzik/Desktop/ETH/Computer Vision/Assignment_5/exercise5_object_recognition/data/data_bow/cars-training-pos/image_0001.png")
# img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
# vPoints = grid_points(img,10,10,8)

# plt.scatter(vPoints[:,1], vPoints[:,0] )
# plt.show()
...
def descriptors_hog(img, vPoints, cellWidth, cellHeight):
    ...
    descriptors = np.empty((0,128))  # list of descriptors for the current image, each entry is one 128-d vector for a grid point

    for i in range(len(vPoints)):
        descriptor = []

        center = [vPoints[i, 0], vPoints[i,1]] # [H,W]

        for j in range(-2, 2):
            for k in range(-2,2):
                his = np.histogram(angles[center[0]+(j*w):center[0]+((j+1)*w), center[1]+(k*h):center[1]+((k+1)*h)], 8)[0]
                descriptor = np.concatenate((descriptor, his))
...
def bow_histogram(vFeatures, vCenters):
    ...
    for i in range(0, vFeatures.shape[0]):
        sums = []

        for k in range(0,vCenters.shape[0]):

            diff = np.square(vFeatures[i,:] - vCenters[k,:])

            sum = np.sum(diff)
            sums.append(sum)

        min_s = min(sums)
        min_idx = sums.index(min_s)

        min_dist.append(min_idx)

    histo = np.bincount(min_dist)
...
def bow_recognition_nearest(histogram,vBoWPos,vBoWNeg):
    ...
    distancesPos = np.zeros((vBoWPos.shape[0],))
    for i in range(0, vBoWPos.shape[0]):

        distancesPos[i] = np.sum(np.square(histogram - vBoWPos[i,:]))

    distancesNeg = np.zeros((vBoWNeg.shape[0]))
    for i in range(0, vBoWPos.shape[0]):
        distancesNeg[i] = np.sum(np.square(histogram - vBoWNeg[i, :]))

    DistPos = min(distancesPos)
    DistNeg = min(distancesNeg)
